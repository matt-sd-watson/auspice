...
class ChooseLayout extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.updateLayout = (layout, scatterVariables=undefined) => {
      if (window.NEXTSTRAIN && window.NEXTSTRAIN.animationTickReference) return;
      this.props.dispatch({
        type: CHANGE_LAYOUT,
        layout,
        scatterVariables: scatterVariables ? {...this.props.scatterVariables, ...scatterVariables} : this.props.scatterVariables
      });
    };
  }
        ...
        <ScatterVariableContainer>
          ...
          <ScatterSelectContainer>
            <Select
              {...miscSelectProps}
              value={selected.x}
              onChange={(value) => this.updateLayout("scatter", {x: value.value})}
            />
          </ScatterSelectContainer>
        </ScatterVariableContainer>
        ...
        <ScatterVariableContainer>
          ...
          <ScatterSelectContainer>
            <Select
              {...miscSelectProps}
              value={selected.y}
              onChange={(value) => this.updateLayout("scatter", {y: value.value})}
            />
          </ScatterSelectContainer>
        </ScatterVariableContainer>
      </>
      ...
      <div style={{marginBottom: 15}}>
            ...
            (
              <RowContainer>
                ...
                <SidebarButton
                  ...
                  onClick={() => this.updateLayout("clock", getStartingScatterVariables(this.props.scatterVariables, this.props.colorings, this.props.distanceMeasure, this.props.colorBy, false))}
                >
                  {t("sidebar:clock")}
                </SidebarButton>
        ...
        <RowContainer>
          ...
          <SidebarButton
            ...
            onClick={() => this.updateLayout("scatter", getStartingScatterVariables(this.props.scatterVariables, this.props.colorings, this.props.distanceMeasure, this.props.colorBy, false))}
          >
            {t("sidebar:scatter")}
          </SidebarButton>
          {selected==="scatter" && (
